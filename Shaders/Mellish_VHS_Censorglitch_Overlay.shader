/*
    Mellish VHS Censorglitch Overlay v1.1 - Normalised Emission
    Created by Mellish
...
Shader "Mellish/VHS Censorglitch/Mellish_VHS_Censorglitch_Overlay"
{
    Properties
    {
        [Header(Main)]
        ...
        _PixelMaskEmission ("CRT Pixel Mask Emission", Range(0,1)) = 0.75
        _PixelMaskEmissionScale ("CRT Pixel Mask Emission Scale", Range(1,30)) = 20
        ...
        _Contrast ("Contrast", Range(0.75,1.15)) = 1.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _PixelMaskEmission;
            float _PixelMaskEmissionScale;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // CRT pixel mask / phosphor emission.
                // Strength controls how much the RGB mask shapes/tints the image.
                // Emission adds a screen-driven glow so the mask can brighten instead of only darkening.
                if (_PixelMaskToggle > 0.5)
                {
                    ...
                    float maskAmount = saturate(_PixelMaskStrength * distanceFade);

                    // Keep the original mask behaviour, but soften it so the image does not become too muddy.
                    float3 maskedCol = col * mask;
                    col = lerp(col, maskedCol, maskAmount);

                    // Add image-driven RGB phosphor glow. The image colour drives the brightness,
                    // while the mask decides which red/green/blue subpixels light up.
                    // UI is normalised 0-1, then scaled internally so the slider feels sensible.
                    float emissionAmount = _PixelMaskEmission * _PixelMaskEmissionScale;
                    float3 phosphorGlow = saturate(col) * mask * (emissionAmount * distanceFade);
                    col += phosphorGlow * maskAmount;
                }
